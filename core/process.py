...
from signal import SIG_DFL, SIGINT
...
from frontend import match, signal_def
...
class Process(Job):
  ...
  def Start(self, why):
    ...
    if pid < 0:
      ...
      pyos.Sigaction(signal_def.GetNumber("PIPE"), SIG_DFL)

      # Respond to Ctrl-\ (core dump)
      pyos.Sigaction(signal_def.GetNumber("QUIT"), SIG_DFL)

      # Child processes should get Ctrl-Z.
      pyos.Sigaction(signal_def.GetNumber("TSTP"), SIG_DFL)

      # More signals from
      # https://www.gnu.org/software/libc/manual/html_node/Launching-Jobs.html
      # (but not SIGCHLD)
      pyos.Sigaction(signal_def.GetNumber("TTOU"), SIG_DFL)
      pyos.Sigaction(signal_def.GetNumber("TTIN"), SIG_DFL)
  ...
  def WaitForOne(self):
    ...
    if pid < 0:  # error case
      ...
      if errno == ECHILD:
        ...
        #log('WaitForOne() => %d', self.trap_state.last_sig_num)
        return self.trap_state.last_sig_num  # e.g. 1 for SIGHUP
      else:
