...
from typing import (
    Dict, Tuple, List, Iterator, Optional, cast, TYPE_CHECKING
)
if TYPE_CHECKING:
...
class Lookup(object):
  ...
  def __init__(self):
    ...
    do_nothing = (_DEFAULT_OPTS, UserSpec([], [], [], DefaultPredicate(), '', ''))  # type: Tuple[Optional[Dict[str, bool]], UserSpec]
    self.lookup = {
...
class RootCompleter(CompletionAction):
  ...
  def Matches(self, comp):
    ...
    debug_f.write('line: %r' % comp.line)
    debug_f.write('rl_slice from byte %d to %d: %r' % (comp.begin, comp.end,
        comp.line[comp.begin:comp.end]))

    debug_f.write('t1 %s' % t1)
    debug_f.write('t2 %s' % t2)
    #debug_f.write('tokens %s', tokens)
    ...
    if len(trail.redirects) > 0:
      ...
      if (r.arg.tag_() == redir_param_e.Word and
        ...
        if WordEndsWithCompDummy(arg_word):
          debug_f.write('Completing redirect arg')

          try:
            val = self.word_ev.EvalWordToString(arg_word)
          except error.FatalRuntime as e:
            debug_f.write('Error evaluating redirect word: %s' % e)
            return
          if val.tag_() != value_e.Str:
            debug_f.write("Didn't get a string from redir arg")
            return
    ...
    if len(trail.words) > 0:
      ...
      if WordEndsWithCompDummy(trail.words[-1]):
        debug_f.write('Completing words')
        #
        ...
        if 0:
          debug_f.write('After tilde detection')
          for w in words2:
            print(w, file=debug_f)

        if 0:
          debug_f.write('words2:')
          for w2 in words2:
            debug_f.write(' %s' % w2)
        ...
        debug_f.write('partial_argv: [%s]' % ','.join(partial_argv))
        num_partial = len(partial_argv)

        first = partial_argv[0]
        alias_first = None # type: str
        if mylib.PYTHON:
          debug_f.write('alias_words: [%s]' % trail.alias_words)
        ...
        if len(trail.alias_words) > 0:
          ...
          try:
            ...
            pass
          alias_first = val.s
          debug_f.write('alias_first: %s' % alias_first)
        ...
        if num_partial == 0:  # should never happen because of Lit_CompDummy
          ...
          span = arena.GetToken(span_id)
          self.comp_ui_state.display_pos = span.col
          self.debug_f.write('** DISPLAY_POS = %d' % self.comp_ui_state.display_pos)
          ...
          self.comp_ui_state.display_pos = span.col
          if mylib.PYTHON:
            self.debug_f.write('words[-1]: [%s]' % trail.words[-1])
    ...
    if not user_spec:
      debug_f.write("Didn't find anything to complete")
      return
    ...
    try:
      ...
      while not done:
        ...
        try:
          ...
          debug_f.write('Got 124, trying again ...')
          done = False
  ...
  def _PostProcess(self,
    ...
    """
    self.debug_f.write('Completing %r ... (Ctrl-C to cancel)' % comp.line)
...
class ReadlineCallback(object):
  ...
  def __call__(self, unused_word, state):
    ...
    try:
      ...
      self.debug_f.write('Runtime error while completing: %s' % e.UserErrorString())
      ...
      self.debug_f.write('Unhandled exception while completing: %s' % e)
