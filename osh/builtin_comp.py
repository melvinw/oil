...
class SpecBuilder(object):
  ...
  def Build(self, argv, complete_attrs, base_opts):
    ...
    for name in complete_attrs.actions:
      if name == 'alias':
...
class CompGen(vm._Builtin):
  ...
  def Run(self, cmd_val):
    ...
    comp.Update('compgen', to_complete, '', -1, [])
    try:
